...
void emf_init(emf_t *emf)
{
  ...
  emf->n3pad = emf->n3+2*emf->nb;/* total number of grid points after padding PML */
...
void extend_model_init(emf_t *emf)
{
  ...
  for(i3=0; i3<emf->n3; i3++){
    for(i2=0; i2<emf->n2; i2++){
  	...
  	t = 2.*emf->omega0;
  	emf->inveps11[i3+emf->nb][i2+emf->nb][i1+emf->nb] = t*emf->rho11[i3][i2][i1];
  	emf->inveps22[i3+emf->nb][i2+emf->nb][i1+emf->nb] = t*emf->rho22[i3][i2][i1];
  	emf->inveps33[i3+emf->nb][i2+emf->nb][i1+emf->nb] = t*emf->rho33[i3][i2][i1];
      }
    }
  }
...
void sanity_check(emf_t *emf)
{
  ...
  if(!getparint("nt", &emf->nt)){
    emf->nt = MAX((emf->n1-1)*emf->d1, (emf->n2-1)*emf->d2)/(emf->vmin*emf->dt);
...
void interpolation_weights(acqui_t *acqui, emf_t *emf, interp_t *interp_rg, interp_t *interp_sg)
{
  ...
  for(irec=0; irec<acqui->nrec; ++irec){
	...
	interp->rec_i3[irec][isub] = j3 + emf->nb;
  ...
  for(isrc=0; isrc< acqui->nsrc; ++isrc){
	...
	interp->src_i3[isrc][isub] = j3 + emf->nb;
...
void airwave_bc_update_H(emf_t *emf)
{
  ...
  for(i2=0; i2<emf->n2fft; i2++){
	...
	emf_kxky[i1+emf->n1fft*i2] = emf->H3[emf->nb][i2][i1] + I*0.; 
      else
  ...
  for(i3=0; i3<emf->rd3; i3++){
    ...
    for(i2=0; i2<emf->n2pad; i2++){
      for(i1=0; i1<emf->n1pad; i1++){
	emf->H1[emf->nb-1-i3][i2][i1] = creal(emf_kxky[i1+emf->n1fft*i2]/(emf->n1fft*emf->n2fft));
      }
    }
  ...
  for(i3=0; i3<emf->rd3; i3++){
    ...
    for(i2=0; i2<emf->n2pad; i2++){
      for(i1=0; i1<emf->n1pad; i1++){
	emf->H2[emf->nb-1-i3][i2][i1] = creal(emf_kxky[i1+emf->n1fft*i2]/(emf->n1fft*emf->n2fft));
      }
    }
  }
...
void airwave_bc_update_E(emf_t *emf)
{
  ...
  for(i2=0; i2<emf->n2fft; i2++){
    for(i1=0; i1<emf->n1fft; i1++){
      if(i1<emf->n1pad && i2<emf->n2pad)
	emf_kxky[i1+emf->n1fft*i2] = emf->E1[emf->nb][i2][i1] + I*0.; 
      else
	emf_kxky[i1+emf->n1fft*i2] = 0. + I*0.;
    }
  }
  ...
  for(i3=0; i3<emf->rd3-1; i3++){
    ...
    for(i2=0; i2<emf->n2pad; i2++){
      for(i1=0; i1<emf->n1pad; i1++){
	emf->E1[emf->nb-1-i3][i2][i1] = creal(emf_kxky[i1+emf->n1fft*i2]/(emf->n1fft*emf->n2fft));
      }
    }
  }
  ...
  for(i2=0; i2<emf->n2fft; i2++){
    for(i1=0; i1<emf->n1fft; i1++){
      if(i1<emf->n1pad && i2<emf->n2pad)
	emf_kxky[i1+emf->n1fft*i2] = emf->E2[emf->nb][i2][i1] + I*0.; 
      else
	emf_kxky[i1+emf->n1fft*i2] = 0. + I*0.;
    }
  }
  ...
  for(i3=0; i3<emf->rd3-1; i3++){
    ...
    for(i2=0; i2<emf->n2pad; i2++){
      for(i1=0; i1<emf->n1pad; i1++){
	emf->E2[emf->nb-1-i3][i2][i1] = creal(emf_kxky[i1+emf->n1fft*i2]/(emf->n1fft*emf->n2fft));
      }
    }
  }
}
...
int check_convergence(emf_t *emf)
{
  ...
  k = 0;
  if(first){
    old[0] = emf->fwd_E1[0][emf->nb][emf->nb][emf->nb];
    old[1] = emf->fwd_E1[0][emf->nb][emf->nb][emf->nb+emf->n1-1];
    old[2] = emf->fwd_E1[0][emf->nb][emf->nb+emf->n2-1][emf->nb];
    old[3] = emf->fwd_E1[0][emf->nb][emf->nb+emf->n2-1][emf->nb+emf->n1-1];
    old[4] = emf->fwd_E1[0][emf->nb+emf->n3-1][emf->nb][emf->nb];
    old[5] = emf->fwd_E1[0][emf->nb+emf->n3-1][emf->nb][emf->nb+emf->n1-1];
    old[6] = emf->fwd_E1[0][emf->nb+emf->n3-1][emf->nb+emf->n2-1][emf->nb];
    old[7] = emf->fwd_E1[0][emf->nb+emf->n3-1][emf->nb+emf->n2-1][emf->nb+emf->n1-1];
    first = 0;
  }else{
    new[0] = emf->fwd_E1[0][emf->nb][emf->nb][emf->nb];
    new[1] = emf->fwd_E1[0][emf->nb][emf->nb][emf->nb+emf->n1-1];
    new[2] = emf->fwd_E1[0][emf->nb][emf->nb+emf->n2-1][emf->nb];
    new[3] = emf->fwd_E1[0][emf->nb][emf->nb+emf->n2-1][emf->nb+emf->n1-1];
    new[4] = emf->fwd_E1[0][emf->nb+emf->n3-1][emf->nb][emf->nb];
    new[5] = emf->fwd_E1[0][emf->nb+emf->n3-1][emf->nb][emf->nb+emf->n1-1];
    new[6] = emf->fwd_E1[0][emf->nb+emf->n3-1][emf->nb+emf->n2-1][emf->nb];
    new[7] = emf->fwd_E1[0][emf->nb+emf->n3-1][emf->nb+emf->n2-1][emf->nb+emf->n1-1];
